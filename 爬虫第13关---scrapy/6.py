...
if soup1.findAll('div',class_='crumbs-nav-item')[2].find('span').text != '':
    gs_typeid = soup1.findAll('div',class_='crumbs-nav-item')[2].find('span').text # 所属类型
else:
    gs_typeid = soup1.findAll('div',class_='crumbs-nav-item')[1].find('span').text

# print(gs_dianpu)

gs_shopcateid = soup1.findAll('div',class_='crumbs-nav-item')[0].find('span').text # 所属栏目
gs_name = soup1.findAll('dd',class_='column')[0].find('span').text[5:] # 商品标题
gs_code = soup1.findAll('dd',class_='column')[1].find('span').text[1:] # 商品编码
gs_weight = soup1.findAll('dd',class_='column')[3].find('span').text[1:3] # 重量
gs_time =  soup1.findAll('dd',class_='column')[4].find('span').text[11:]  # 上架时间
print(gs_name)

# for i in imgs:
#     html = etree.HTML(str(i))
#     piclist = html.xpath("//li[@data-type='img']/a/@href")
#     piclist1 = html.xpath("//li[@data-type='img']/a/img/@src")
#     piclist += piclist1
